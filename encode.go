...
var (
	...
	MarshalTypeErr = errors.New("only support struct type")
)
...
func writeString(e *encodeState, value reflect.Value) error {
	if value.Type().Name() == String32Name {
		if err := WriteI32(e, reflect.ValueOf(len(value.String()))); err != nil {
			return err
		}
	} else {
...
func Marshal(uri uint32, v interface{}) ([]byte, error) {
	// 强制所有marshal的对象全部都为结构体来避免各种不可预知的行为
	pkgType := reflect.TypeOf(v)
	if pkgType.Kind() != reflect.Struct {
		return nil, MarshalTypeErr
	}

	e := &encodeState{}

	// len
	lenBuff := e.scratch[0:4]
	binary.LittleEndian.PutUint32(lenBuff, 0) // 先写一个0，后面再改
	if _, err := e.Write(lenBuff); err != nil {
		return nil, err
	}

	// uri
	uriBuff := e.scratch[0:4]
	binary.LittleEndian.PutUint32(uriBuff, uri)
	if _, err := e.Write(uriBuff); err != nil {
		return nil, err
	}

	// rescode
	resBuff := e.scratch[0:2]
	binary.LittleEndian.PutUint16(resBuff, uint16(200))
	if _, err := e.Write(resBuff); err != nil {
		return nil, err
	}
	...
	// 重新计算包长度
	// 用4个字节替换4个字节，应该不会引起内存重新分配导致没有写到buffer
	binary.LittleEndian.PutUint32(lenBuff, uint32(e.Len()))
	copy((e.Bytes())[0:4], lenBuff)

	return e.Bytes(), nil
}

// 有的时候需要marshall一些结构体到buff，但是不添加header的情况
