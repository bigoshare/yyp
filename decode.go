...
type String32 string // yy协议里的string长度都是16位保存的。该类型用于特殊的32位长度的string字段
const String32Name = "String32"
...
func (e *DecodeState) readString32() string {
	len := e.ReadU32()
...
func (d *DecodeState) unmarshal(data []byte, v interface{}) (err error) {
	...
	case reflect.String:
		if elem.CanSet() {
			if elem.Type().Name() == String32Name { // 长度为32位
				elem.SetString(d.readString32())
			} else {
